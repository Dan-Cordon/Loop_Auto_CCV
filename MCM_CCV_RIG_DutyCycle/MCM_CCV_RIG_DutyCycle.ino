...
#define DUTY_CYCLE_PERIOD_MS 1000    // Total cycle time (2 seconds)
...
void manageMotorControl(float target, bool vibration) {
    ...
    if (target <= 0) {
        ...
        // This ensures that when you start again, it ramps up from 0, 
        // rather than thinking it's still at the old speed.
    ...
    // ---------------------------------------------------------
    // We only ramp if we are in the continuous zone (> MIN_STABLE_RPM).
    // If we are in duty cycle mode, we skip this to ensure precise burst timing.
    ...
    if (target >= MIN_STABLE_RPM) {
        ...
        // Ramp DOWN (Only used if changing speed, e.g., 100 -> 50. NOT used for Stop.)
    ...
    // B. Low Speed Duty Cycle (Bursts of 10 RPM)
        ...
        // NOTE: We use the ORIGINAL 'target' for the math, not the ramped one.
        ...
        if (currentMillis - lastCycleStart < onTime) {
            setMotorSpeed(MIN_STABLE_RPM); // Safe "Jump" to 10 RPM
            if (vibration) gpio_set_level(VIBRATION_PIN, 1);
            isDutyCycleActive = true;
        } else {
            setMotorSpeed(0); // Instant Off for duty cycle pause
            // if (vibration) gpio_set_level(VIBRATION_PIN, 0);
        }
    }
}
...
// --- Main Loop ---
void loop() {
    // 1. Read Serial Commands (Existing code remains the same...)
    ...
    if (serialControlActive) {
        ...
        if (triggerActive) {
            activeRPM = MOTOR_RPM;
            activeVib = true; // Manual always vibrates based on original code
        } else {
            activeRPM = 0;
            activeVib = false;
        }
    ...
    // Delegate to the new manager to handle low-speed bursting
    manageMotorControl(activeRPM, activeVib);

    // 3. Output Data
    // Note: If duty cycling, currentRate might fluctuate slightly depending on window size
    Serial.printf("Mass:%.2f,Rate:%.2f,RPM:%.0f\n", currentMass, currentRate, targetRPM);
    
    // REDUCED DELAY: 50ms is too slow for accurate timing control. 
    // Reduced to 10ms to catch the On/Off windows accurately.
